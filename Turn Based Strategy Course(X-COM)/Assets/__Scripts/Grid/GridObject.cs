                        ...
                        // GridObject создается в каждой ячейки сетки. Является оболочкой для хранения Юнитов
{
    ...
    private GridSystemHexAndQuad<GridObject> _gridSystem; // Сеточная система .В дженерик предаем тип GridObject// Частная сеточная система которая создала этот объект (это расширение например для сетки 2-го этажа)
    ...
    public GridObject(GridSystemHexAndQuad<GridObject> gridSystem, GridPosition gridPosition) // Конструктор 
    {
